...
describe('SafePalAdapter', () => {
  describe('with QR provider', () => {
    ...
    it('should connect via QR', async () => {
      await expect(adapter.connect(TransportMethod.QR)).resolves.toBeUndefined();
    });
    ...
    it('should sign transaction via QR flow', async () => {
      ...
      const deriveResponse = JSON.stringify({ status: 'ok', result: MOCK_ADDRESS });
      mockQR.scanQR
        .mockResolvedValueOnce(JSON.stringify({ status: 'ok', result: MOCK_SIGNATURE_HEX }))
    ...
    it('should handle error responses', async () => {
      ...
      await adapter.connect(TransportMethod.QR);
      await expect(
        adapter.deriveAccount("m/44'/501'/0'/0'")
      ).rejects.toThrow();
    });
  });
